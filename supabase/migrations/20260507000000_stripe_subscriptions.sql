...
-- Stripe / subscription columns (and user_id if missing)
...
drop policy if exists "Users can read own profile" on public.rvn_profiles;
create policy "Users can read own profile"
  on public.rvn_profiles for select
  using (auth.uid() = user_id);

drop policy if exists "Users can update own profile" on public.rvn_profiles;
create policy "Users can update own profile"
  on public.rvn_profiles for update
  using (auth.uid() = user_id);

drop policy if exists "Users can insert own profile" on public.rvn_profiles;
